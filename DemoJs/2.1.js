...
//  коли інтерпретатор зустрічає повернення,
//  і весь код після нього буде проігнорований в поточному виклику функції.

// Візьмемо вже знайому нам функцію перевірки повноліття.
//  Вона працює, але тут є «зайвий» код, тобто тіло функції можна оптимізувати.
//  В цьому випадку підійде спосіб (патерн) «раннє повернення».
...
//  повертаємо рядок "You are an adult", і код нижче вже не виконається.
// Якщо умова в if не виконується,
//  тобто приводиться до false, повертаємо рядок "You are a minor".
// Використовуючи патерн «раннє повернення» 
// і те, що виконання функції переривається
//  на операторі return, ми позбавляємося зайвої змінної і блоку else.
//  Тобто цей спосіб допомагає «розгладити» розгалуження.
