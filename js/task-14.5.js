// Геттеры и сеттеры
// Геттеры и сеттеры - это более краткий синтаксис
//  объявления методов для взаимодействия со свойствами.
...
// Геттеры и сеттеры хорошо использовать для простых
//  операций чтения и изменения значения свойств,
...
//   }
// }
// Мы объявили геттер и сеттер email поставив
//  перед именем свойства ключевые слова get и set.
//  Внутри этих методов мы или возвращаем значение
//  приватного свойства #email или изменяем его значение.
...
// При обращении к mango.email вызывается геттер get email() {...} 
// и выполняется его код. При попытке записи mango.email = 'mango@supermail.com'
//  вызывается сеттер set email(newEmail) {...} и строка 'mango@supermail.com'
//  будет значением параметра newEmail.

// Плюс в том, что это методы, а значит при записи можно выполнить
//  дополнительный код, например с какими-то проверками, в отличии
//  от выполнениях этой же операции напрямую со свойством.
...
//  публичный интерфейс класса заменив уже объявленные методы
